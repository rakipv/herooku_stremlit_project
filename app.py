...
#########################
#
# df = pd.read_csv("R:/faculty_recommendatio/SBU_captioned_photo_dataset_urls.csv")  # read a CSV file inside the 'data" folder next to 'app.py'
...
if function_option == "Get recommendations by Journal name":
    def recommend(movie):
        ...
        for i in movies_list:
            ...
            faculty_id = movies.iloc[i[0]].faculty_id
            # recommended_movie_posters.append(fetch_poster(faculty_id))
            recommended_movie_names.append(movies.iloc[i[0]].title)

        return recommended_movie_names, recommended_movie_posters


    movies_dict = pickle.load(open('R:/faculty_recommendatio/movie_dict.pkl', 'rb'))
    similarity = pickle.load(open('R:/faculty_recommendatio/similarity.pkl', 'rb'))
...
if function_option == "Get recommendations by Journal Author":
    def recommend_director(director):
        ...
        distances = similarity[movie_index]
        movies_list = sorted(list(enumerate(distances)), multi=True,
                             reverse=True, key=lambda x: x[1])[1:6]
        recommended_movie_names = []
        recommended_movie_posters = []
        for i in movies_list:
    ...
    if st.button('Get Recommendation'):
        recommended_movie_names, recommended_movie_posters = recommend_director(
            selected_movie)
        col1, col2, col3 = st.columns(3)
        image = Image.open('R:/faculty_recommendatio/Google_Scholar_logo.svg.png')
